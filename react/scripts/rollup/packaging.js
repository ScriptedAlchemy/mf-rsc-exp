...
function resolveCustomOutputPath(bundle, filename, packageName) {
  if (bundle.outputDir) {
    const base = path.isAbsolute(bundle.outputDir)
      ? bundle.outputDir
      : path.join(`build/node_modules/${packageName}`, bundle.outputDir);
    if (path.isAbsolute(bundle.outputDir)) {
      workspacePackageTargets.set(packageName, path.dirname(base));
    }
  ...
  if (path.isAbsolute(bundle.outputPath)) {
    return bundle.outputPath;
  }
  return `build/node_modules/${packageName}/${bundle.outputPath}`;
}

function getBundleOutputPath(bundle, bundleType, filename, packageName) {
  const customPath = resolveCustomOutputPath(bundle, filename, packageName);
  if (customPath) {
    return customPath;
  }
  switch (bundleType) {
    ...
    case BROWSER_SCRIPT: {
      throw new Error(
        'Bundles with type BROWSER_SCRIPT must specify an explicit output path.'
      );
    }
    default:
...
async function prepareWorkspacePackages() {
  if (workspacePackageTargets.size === 0) {
    return;
  }
  await Promise.all(
    Array.from(workspacePackageTargets.entries()).map(([name, distRoot]) =>
      asyncCopyTo(`packages/${name}/npm`, distRoot)
    )
  );
}
